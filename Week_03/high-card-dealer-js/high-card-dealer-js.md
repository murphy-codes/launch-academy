...
```no-highlight
et get high-card-dealer-js
cd high-card-dealer-js
yarn install
node src/game.js
```
...
Create a console-based game where 52 cards are created, and two players are dealt four cards randomly from the deck. The player that has the highest total value of cards in their hand wins the game. The game should output information to the command line accordingly, so that we know which player won the game and why.

Refreshing your browser console should run the game to completion. No user input is needed for this application.

## Order of the Game

- A user can start the game by running `node src/Game.js` and opening the dev console.
...
Because running `Game.js` by refreshing the console starts the game, you should have some, if not a majority of your logic in this file, such as a creating your `Deck` class and `Hand` class. You are encouraged to brainstorm, or better yet, discuss with others, how to get started.
...
The logic in the `Deck` should already allow for the creation of a 52 unique cards. Specifically, the `buildDeck` method uses our lists of ranks and suits to create `Card` objects programmatically. Because `buildDeck` is called in our constructor, any new `Deck` object will come with 52 cards.
...
- Create Jest tests for each of our classes and their methods. Note that if your methods are too long, or if they have too much logic, they may be more difficult to test. As such, you may need to refactor some of your code to create DRYer methods.
